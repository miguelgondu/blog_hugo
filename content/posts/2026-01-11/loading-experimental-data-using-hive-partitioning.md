---
date: "2026-01-10"
title: "Structuring experimental data using hive partitioning and duckdb"
description: If you structure your experimental data in a certain way, you get the loading almost for free using duckdb.
summary: If you structure your experimental data in a certain way, you get the loading almost for free using duckdb.
images: null
image: null
---

> This blogpost has a code companion. [Check it out here!]()

I learned a trick from [a friend called Oscar](https://github.com/odarbelaeze) recently: if you organize your experiments in a certain way, **you can easily load your results as pandas dataframes**. This is enabled by a combination of hive partitioning and [duckdb](https://duckdb.org/docs/stable/data/partitioning/hive_partitioning).

# Probably your current setup

Imagine you're running an experiment, and your experimental config has several variables. For example, you could be running a computational analysis on how a molecule binds to a protein, and you might want to run it for several proteins, molecules, and seeds.
...
Then, when you're loading them up using pandas, you might be either loading them up and concatenating them or, even worse, parsing the filename and extracting the protein, molecule and seed from there using ugly `name.split(_)` shenanigans.
...
def load_results() -> pd.DataFrame:
    ...
    FROM read_csv("data/**/*.csv", hive_partitioning=true)
    """
...
You could even flex your SQL skills by running the computations you need (e.g. filtering, computing averages over seeds...) inside the SQL query itself.
